...
OPTIONS=`getopt -o '' -l ips-only,days: -- "$@"`
eval set -- "$OPTIONS"
...
cmd_str="ahoy platform log:access $days --ip --extra '${cmd_extra//\'/\'\"\'\"\'}' $@"
...
if [ -n "$iponly" ]; then
  echo "$ips" | awk '{ if ($1 > '${PLATFORMSH_RECIPES_IPBLOCK_THRESHOLD-20}') print $1 "," $2}'
  exit
fi
